...
class CollectionSplitDiagnosticsTest extends TestCase
{
    ...
    public function test_it_reports_one_binary_multifile_split_cohorts_for_a_group(): void
    {
        ...
        $this->assertSame(1, $summary['cohorts'][0]['duplicate_filenumbers']);
        $this->assertSame(66.67, $summary['cohorts'][0]['filenumber_coverage_percent']);
        $this->assertSame('1,2', $summary['cohorts'][0]['filenumber_span']);
        $this->assertSame(500101, $summary['cohorts'][0]['xref_min_article']);
        $this->assertSame(500103, $summary['cohorts'][0]['xref_max_article']);
        $this->assertSame(3, $summary['cohorts'][0]['xref_article_span']);
        $this->assertSame(3, $summary['cohorts'][0]['xref_articles']);
        $this->assertSame(0, $summary['cohorts'][0]['xref_missing_articles']);
        $this->assertSame(100.0, $summary['cohorts'][0]['xref_article_coverage_percent']);
        $this->assertSame(120, $summary['cohorts'][0]['date_span_seconds']);
    ...
    public function test_artisan_command_can_emit_cohorts_only_json_diagnostics(): void
    {
        ...
        $this->assertSame(1, $output['cohorts'][0]['duplicate_filenumbers']);
        $this->assertSame(3, $output['cohorts'][0]['xref_article_span']);
    }

    public function test_mariadb_cohort_query_projects_xref_article_alias(): void
    {
        config([
        ...
        $method = new ReflectionMethod(CollectionSplitDiagnosticsService::class, 'cohorts');
        $method->setAccessible(true);

        $queries = DB::connection()->pretend(function () use ($method): void {
            $method->invoke(new CollectionSplitDiagnosticsService, 5, 'alt.binaries.blu-ray', 1);
        });

        $this->assertNotEmpty($queries);
        $this->assertStringContainsString('SUBSTRING_INDEX(SUBSTRING_INDEX(c.xref', $queries[0]['query']);
        $this->assertStringContainsString('AS xref_article', $queries[0]['query']);
    }
    ...
    private function seedSplitCollections(): void
    {
        ...
        $this->insertCollection(101, 5, '[1/3] - "aaa.bin" yEnc', 'poster-a', 3, 88, 'batch-a', 1, 80, '2026-06-14 05:50:00', 500101);
        $this->insertCollection(102, 5, '[2/3] - "bbb.bin" yEnc', 'poster-b', 3, -10, 'batch-a', 2, 82, '2026-06-14 05:51:00', 500102);
        $this->insertCollection(103, 5, '[2/3] - "ccc.bin" yEnc', 'poster-c', 3, 88, 'batch-a', 2, 82, '2026-06-14 05:52:00', 500103);
    ...
    private function insertCollection(
        ...
        int $fileNumber,
        int $totalParts,
        string $date = '2026-06-14 05:50:00',
        ?int $xrefArticle = null
    ): void {
        DB::table('collections')->insert([
            'id' => $id,
            'subject' => $subject,
            'fromname' => $poster,
            'date' => $date,
            'xref' => 'alt.binaries.blu-ray:'.($xrefArticle ?? $id),
            'groups_id' => $groupId,
            'totalfiles' => $totalFiles,
            'collectionhash' => 'hash-'.$id,
            'collection_regexes_id' => $regexId,
