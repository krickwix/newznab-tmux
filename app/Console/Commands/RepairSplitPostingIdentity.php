...
final class RepairSplitPostingIdentity extends Command
{
    protected $signature = 'nntmux:repair-split-posting-identity
                            ...
                            {--posting= : Repair only this exact cleaned posting stem}
                            {--update : Apply the regrouping; default is dry-run}
                            {--json : Emit machine-readable JSON}';

    protected $description = 'Reunite per-file collections that ingest split by part count (dry-run by default)';

    public function handle(SplitPostingIdentityRepairService $service): int
    {
        try {
        ...
        $this->table(
            ...
            [
                ...
                ['binaries removed (duplicate files)', $summary['binaries_removed']],
        ...
        if ($summary['binaries_with_duplicate_partnumbers'] > 0) {
            $this->warn(sprintf(
                '%d binary/binaries already hold two parts sharing a partnumber. That is '
                .'pre-existing damage from ingest, not something this merge causes or fixes; '
                .'the affected files may still be short of their declared part count.',
                $summary['binaries_with_duplicate_partnumbers']
            ));
        }
        ...
        if ($summary['cohort_limit_reached']) {
            $this->warn('The cohort limit was reached; more split postings remain. Re-run to continue.');
        }
